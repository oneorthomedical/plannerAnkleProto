    ...
    constructor(preplanification) {
        ...
        this._objectPromise = new Promise(function (resolve, reject) {
            ...
            const progress = function ( xhr ) {
                $('#progressFibula').width(`${Math.round(xhr.loaded / xhr.total * 100 )}%`);
                $('#progressFibula').html(`${Math.round(xhr.loaded / xhr.total * 100 )}%`);
                let count = $(('#countFibula'));
                count.text(Math.ceil(Math.round(xhr.loaded / xhr.total * 100 ))+ "%");
                let s = Snap('#animatedFibula');
                let progress = s.select('#progressFibbula');
                progress.attr({ 'stroke-dasharray':Math.round(xhr.loaded / xhr.total * 100 )*2.512+',251.2'});
            };
            loader.load(`${scope.preplanification.path}/perone.obj`, function (obj) {
                obj.traverse(function (child) {
                    if (child instanceof THREE.Mesh) {
                        let mesh = new THREE.Mesh(scope.assignUVs(new THREE.Geometry().fromBufferGeometry(child.geometry)), mlib.TextureFibula );
                        mesh.name = `Perone`;
                        resolve(mesh);
                    }
                })
