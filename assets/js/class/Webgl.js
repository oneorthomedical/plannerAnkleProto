import {
    ...
    cylindreRed,
    perone,
    talusCalcaneumRecalage,
    tibiaRecalage,
    radio
} from './objectDeclaration.js';
import {pathToLink, browser} from "./Version";
...
export default class Webgl {
    ...
    initContent() {
        Promise.all([
             ...
             */
            if ((dataResponse.User.profil === "administrator" || dataResponse.User.profil === "provider") && dataResponse.Planning.state !== 0) {
            ...
            $('.varusValgusTi').slider({
                change: (event, ui) => {
                    if (preplanification.change) {
                        ...
                        cylindreGreen.updateVarus(ui.value);
                        os.update(mortaiseTalaire, mortaiseTibia, parentGroupTalaire);
                    }
                }
            });
            $('.angleSagittalTi').slider({
                change: (event, ui) => {
                    if (preplanification.change) {
                        ...
                        cylindreGreen.updateSagittal(ui.value);
                        os.update(mortaiseTalaire, mortaiseTibia, parentGroupTalaire);
                    }
                }
            });
            $('.angleMMTi').slider({
                change: (event, ui) => {
                    if (preplanification.change) {
                        ...
                        cylindreGreen.updateTilt(ui.value);
                        os.update(mortaiseTalaire, mortaiseTibia, parentGroupTalaire);
                    }
                }
            });
            ...
            if (jsonCompute.getLang() === 'fr' && browser === 'firefox') {
                if (window.matchMedia("(min-width: 1350px)").matches) {
                    $('.directionViews').css('left', '106%');
                } else {
                    $('.directionViews').css('left', '108%');
                }
            } else if (jsonCompute.getLang() === 'en' && browser === 'firefox') {
                console.log('css compute', 'en + firefox')
                if (window.matchMedia("(min-width: 1350px)").matches) {
                    $('.directionViews').css('left', '109%');
                } else {
                    $('.directionViews').css('left', '112%');
                }
            ...
            checkboxRadio.addEventListener('change', function () {
                ...
                if (this.checked) {
                    ...
                    const canvasRadioFace = document.getElementById('radioFaceView');
                    const canvasViewFace = document.getElementById('faceView');
                    imgFace.onload = function () {
                        canvasRadioFace.width = canvasViewFace.width;
                        canvasRadioFace.height = canvasViewFace.height;
                        const coeffWidth = canvasViewFace.width / this.width;
                        const coeffHeight = canvasViewFace.height / this.height;
                        const height = this.height * coeffWidth;
                        const deltaWidth = Math.abs(this.width * coeffHeight - canvasViewFace.width)
                        console.log("width 002 ", this.width * coeffHeight, canvasViewFace.width , deltaWidth);
/*
                        canvasRadioFace.style.height = `${height}px`;
*/
                        ctxFace.drawImage(imgFace, 0, 0, canvasViewFace.width , canvasViewFace.height);
                    };
                    ...
                    const canvasRadioProfil = document.getElementById('radioProfilView');
                    const canvasViewProfil = document.getElementById('profilView');
                    imgProfil.onload = function () {
                        canvasRadioProfil.width = canvasViewProfil.width;
                        canvasRadioProfil.height = canvasViewProfil.height;
                        const coeff = canvasViewFace.width / this.width;
                        const height = this.height * coeff;
                        console.log("height 002 ", this.height, height);
                       /* canvasRadioProfil.style.height = `${height}px`;*/
                        ctxProfil.drawImage(imgProfil, 0, 0, canvasRadioProfil.width, canvasViewProfil.height);
                    };
            ...
            $('#exportStl').click(() => {
                let utils = new Utils();
                let talaireStl = utils.exportToStl(talaire.mesh);
                let tibiaBrut = tibia.mesh.clone();
                tibiaBrut.remove(tibiaBrut.children[0]);
                let tibiaStl = utils.exportToStl(tibiaBrut);
                utils.saveData(talaireStl, 'talaire');
                utils.saveData(tibiaStl, 'tibia');
                console.log(utils);
            });
            $('#exportStl').click(() => {
                let utils = new Utils();
